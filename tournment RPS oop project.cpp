/*
 * ROCK-PAPER-SCISSORS TOURNAMENT MANAGER (OOP Redesign)
 *
 * A console application replicating the core concept of a tournament management website.
 * Features teams, players, quick matches (1v1 / 1vAI), single-elimination tournaments,
 * match history, rankings, past tournament archive, settings, and file persistence.
 *
 * OOP Enhancements:
 *  - Abstract Match base class with QuickMatch and TournamentMatch subclasses (polymorphism)
 *  - Strategy pattern for input mode (IMoveProvider interface)
 *  - Abstract persistence interface (IPersistence) with file implementation (Dependency Inversion)
 *  - Better encapsulation and single responsibility
 *
 * Compile with C++17 – no external libraries needed.
 * All ASCII art, file I/O, and game logic are included in this single source file.
 */

 // ──────────────────────────────────────────────
 //  HEADERS & GLOBAL DEFINES
 // ──────────────────────────────────────────────
#include <iostream>
#include <string>
#include <vector>
...
// ANSI colour codes (if terminal supports)
#define RESET   "\033[0m"
#define BOLD    "\033[1m"
#define RED     "\033[31m"
...
// ──────────────────────────────────────────────
//  CROSS-PLATFORM HIDDEN CHARACTER INPUT
// ──────────────────────────────────────────────
char getHiddenChar() {
#ifdef _WIN32
    return _getch();
...
public:
    std::string name;
    std::string icon;       // short Unicode string
    int wins;
...
struct MatchHistory {
    MatchType type;
    std::string round;      // e.g. "Round 1", "Final" (empty for quick matches)
    std::string side1;
    std::string side2;
    std::string winner;
    int score1;
...
// ──────────────────────────────────────────────
//  PERSISTENCE INTERFACE (Dependency Inversion)
// ──────────────────────────────────────────────
class IPersistence {
public:
    virtual ~IPersistence() = default;
    virtual void saveTeams(const std::vector<Team>& teams) = 0;
    virtual void loadTeams(std::vector<Team>& teams) = 0;
    virtual void saveHistory(const std::vector<MatchHistory>& history) = 0;
    virtual void loadHistory(std::vector<MatchHistory>& history) = 0;
    virtual void saveSettings(const std::string& inputMode) = 0;
    virtual void loadSettings(std::string& inputMode) = 0;
    ...
    virtual void saveArchive(const std::vector<TournamentArchive>& archive) = 0;
    virtual void loadArchive(std::vector<TournamentArchive>& archive) = 0;
};

class FilePersistence : public IPersistence {
public:
    void saveTeams(const std::vector<Team>& teams) override {
    ...
    void loadTeams(std::vector<Team>& teams) override {
    ...
    void saveHistory(const std::vector<MatchHistory>& history) override {
    ...
    void loadHistory(std::vector<MatchHistory>& history) override {
    ...
    void saveSettings(const std::string& inputMode) override {
        std::ofstream out("settings.txt");
        if (!out) return;
        out << "input_mode=" << inputMode << "\n";
    }

    void loadSettings(std::string& inputMode) override {
    ...
    void saveArchive(const std::vector<TournamentArchive>& archive) override {
    ...
    void loadArchive(std::vector<TournamentArchive>& archive) override {
...
// ──────────────────────────────────────────────
//  MOVE INPUT STRATEGY (Strategy Pattern)
// ──────────────────────────────────────────────
class IMoveProvider {
public:
    virtual ~IMoveProvider() = default;
    virtual char getMove(const std::string& playerName) = 0;
};

class HiddenMoveProvider : public IMoveProvider {
public:
    char getMove(const std::string& playerName) override {
        while (true) {
            std::cout << playerName << ", enter your move (R/P/S): ";
            char move = getHiddenChar();
            move = toupper(move);
            if (move == 'R' || move == 'P' || move == 'S') {
                std::cout << "\n";  // confirm without showing the character
                return move;
            }
...
class ClearScreenMoveProvider : public IMoveProvider {
public:
    char getMove(const std::string& playerName) override {
        while (true) {
            std::cout << playerName << ", enter your move (R/P/S): ";
...
// ──────────────────────────────────────────────
//  ABSTRACT MATCH CLASS (Polymorphism)
// ──────────────────────────────────────────────
struct MatchResult {
    std::string winner;
    int score1, score2;
...
public:
    QuickMatch(const std::string& p1, const std::string& p2, bool ai, IMoveProvider& mp)
        : player1(p1), player2(p2), isAI(ai), moveProvider(mp) {
    }
    ...
    MatchResult play() override {
        ...
        while (score[0] < 2 && score[1] < 2) {
            std::cout << "\nRound " << roundNum << ":\n";
            char move1 = moveProvider.getMove(player1);
            char move2 = isAI ? aiGen.getRandomMove() : moveProvider.getMove(player2);
...
// ──────────────────────────────────────────────
//  TOURNAMENT MATCH IMPLEMENTATION
// ──────────────────────────────────────────────
class TournamentMatch : public Match {
    ...
    IMoveProvider& moveProvider;
    RandomMoveGenerator aiGen; // not used in tournament (no AI), but needed for consistency
public:
    TournamentMatch(const std::string& ta, const std::string& tb,
        const std::string& pa, const std::string& pb,
        bool isBye, IMoveProvider& mp)
        : teamA(ta), teamB(tb), playerA(pa), playerB(pb),
        bye(isBye), moveProvider(mp) {
    }
    ...
    MatchResult play() override {
        ...
        while (score[0] < 2 && score[1] < 2) {
            std::cout << "\nRound " << roundNum << ":\n";
            char move1 = moveProvider.getMove(playerA + " (" + teamA + ")");
            char move2 = moveProvider.getMove(playerB + " (" + teamB + ")");
            int result = compareMoves(move1, move2);
            if (result == 0) std::cout << "Tie!\n";
            else if (result == 1) {
...
// ──────────────────────────────────────────────
//  TEAM MANAGER (with persistence injection)
// ──────────────────────────────────────────────
class TeamManager {
private:
...
public:
    TeamManager(IPersistence& pers) : persistence(pers) {
...
private:
    ...
    IPersistence& persistence;
public:
    HistoryManager(IPersistence& pers) : persistence(pers) {
...
public:
    Tournament(TeamManager& tm, HistoryManager& hm, IMoveProvider& mp, IPersistence& pers)
        : teamManager(tm), historyManager(hm), moveProvider(mp), persistence(pers) {
    }
    ...
    bool start(const std::vector<Team>& allTeams) {
        ...
        while (bracket.size() > 1) {
            ...
            for (size_t i = 0; i < bracket.size(); i += 2) {
                ...
                TournamentMatch match(sideA, sideB, playerA, playerB, byeMatch, moveProvider);
                match.setRoundDescription("Round " + std::to_string(roundNum));
                MatchResult res = match.play();

                if (!byeMatch) {
                    // Update team wins for the winner (team name, not player name)
        ...
        champion = bracket[0];
        std::cout << BOLD << GREEN << "\n=== TOURNAMENT CHAMPION: " << champion << " ===\n" << RESET;        archiveThisTournament();
        return true;
    }

private:
    void archiveThisTournament() {
        IPersistence::TournamentArchive arc;
        ...
        std::vector<IPersistence::TournamentArchive> archive;
...
class Application {
private:
    FilePersistence filePersistence;      // Concrete persistence (owns)
    TeamManager teamManager;
    HistoryManager historyManager;
    std::unique_ptr<IMoveProvider> moveProvider; // Strategy
    std::string inputMode; // "hidden" or "clear"
    bool running;

public:
    Application() : teamManager(filePersistence),
        historyManager(filePersistence),
        running(false) {
    ...
    void run() {
        ...
        // Final persistence save handled by destructors
    }

private:
    void setMoveProvider() {
        if (inputMode == "hidden")
            moveProvider = std::make_unique<HiddenMoveProvider>();
        else
            moveProvider = std::make_unique<ClearScreenMoveProvider>();
    }
    ...
    void addPlayer() {
        const auto& teams = teamManager.getTeams();
        if (teams.empty()) { std::cout << "No teams. Create one first.\n"; return; }
        ...
        if (pname.empty()) { std::cout << "Name cannot be empty.\n"; return; }
    ...
    void quickMatch() {
        ...
        QuickMatch match(player1, player2, isAI, *moveProvider);
        MatchResult res = match.play();
    ...
    void startTournament() {
        Tournament t(teamManager, historyManager, *moveProvider, filePersistence);
        t.start(teamManager.getTeams());
    }

    void showRankings() const {
        auto ranked = teamManager.getRankings();
        if (ranked.empty()) { std::cout << "No teams.\n"; return; }
    ...
    void showArchive() {
        ...
        if (archive.empty()) { std::cout << "No past tournaments.\n"; return; }
        for (const auto& arc : archive) {
